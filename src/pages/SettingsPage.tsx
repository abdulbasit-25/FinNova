...
export default function SettingsPage() {
  ...
  const handleExport = useCallback(() => {
    ...
    downloadFile(`expense-tracker-backup-${stamp}.json`, exportData(), 'application/json');
    toast({ title: 'Success', description: 'Backup data downloaded successfully!' });
  }, [exportData, toast]);
    ...
    >
      ...
      <div className="glass-card p-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex gap-3 mt-2" role="group" aria-labelledby="accent-color-label">
            {ACCENT_COLORS.map((c) => (
              <button
                key={c.value}
              ...
              >
                <span className="h-4 w-4 rounded-full" style={{ backgroundColor: `hsl(${c.hsl})` }} aria-hidden="true" />
                {c.label}
              </button>
            ))}
          </div>
      ...
      <AlertDialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete all your expenses, categories, budgets,
            goals, and accounts. Please make sure you have a backup before proceeding.
          </AlertDialogDescription>
