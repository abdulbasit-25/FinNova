...
export default function Budgets() {
  ...
  const overallBudget = useMemo(
    () => monthBudgets.find((b) => b.categoryId === null),
    [monthBudgets],
  );
  const categoryBudgets = useMemo(
    () => monthBudgets.filter((b) => b.categoryId !== null),
    [monthBudgets],
  );
  ...
  const overallPct = overallBudget
    ? pctOf(totalExpenses, overallBudget.amount)
    : 0;
  const overallOver = overallBudget
              ...
              >
                {categoryBudgets.map((b) => {
                  ...
                  const spent = b.categoryId
                    ? (categorySpend.get(b.categoryId) ?? 0)
                    : 0;
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800 dark:text-blue-200 mt-0.5">
              Set both overall and category-specific budgets. Monitor spending
              regularly to stay within limits and achieve your financial goals.
            </p>
          </div>
        </div>
      </Card>
