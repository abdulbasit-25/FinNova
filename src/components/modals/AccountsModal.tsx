...
import { AlertCircle, DollarSign, Building, CreditCard, Wallet, Pin, Check } from 'lucide-react';
...
export function AccountsModal({
  ...
  const [color, setColor] = useState(editingAccount?.color || 'hsl(199, 89%, 48%)');
  const [type, setType] = useState<'cash' | 'bank' | 'credit_card' | 'wallet' | 'custom'>(
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Select value={type} onValueChange={v => setType(v as any)} disabled={isSubmitting}>
            ...
            <SelectContent>
              ...
              <SelectItem value="bank"><div className="flex items-center gap-2"><Building className="h-4 w-4" /><span>Bank Account</span></div></SelectItem>
              <SelectItem value="credit_card"><div className="flex items-center gap-2"><CreditCard className="h-4 w-4" /><span>Credit Card</span></div></SelectItem>
              <SelectItem value="wallet"><div className="flex items-center gap-2"><Wallet className="h-4 w-4" /><span>Digital Wallet</span></div></SelectItem>
              <SelectItem value="custom"><div className="flex items-center gap-2"><Pin className="h-4 w-4" /><span>Custom</span></div></SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {icon && color && (
          <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/50 border border-border">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">Balance: ${Number(balance).toFixed(2)}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-4 border-t border-border">
          ...
          <Button
            ...
            className="flex-1"
          >
            {isSubmitting ? 'Adding...' : editingAccount ? 'Update' : 'Add'} Account
          </Button>
        </div>
      </form>
    </BaseModal>
